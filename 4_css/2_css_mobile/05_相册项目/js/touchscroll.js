(function (w) {
     ...
     * @param callback 对象， 属性：指定回调函数  {start, move, end, complete}
     */
    w.touchscroll = function(app, content, scrollBar, callback){

        var isTouch = true;  //是否允许触发touch

        if (scrollBar) {
            // 计算比例
            content.sliderScale = app.clientHeight / content.offsetHeight;
            //如果比例>=1
            if (content.sliderScale >= 1) {
                scrollBar.style.display = 'none';
            }
            // 计算滚动条高度
            scrollBar.style.height = app.clientHeight * content.sliderScale + 'px';
            // 强制把滚动条的不透明·度设置为0
        ...
        content.addEventListener('touchmove', function (event) {
            ...
            if (scrollBar) {
                ...
                transformCss(scrollBar, 'translateY', -translateY * content.sliderScale);
            }
        ...
        content.addEventListener('touchend', function(event){
            ...
            if (scrollBar) {
                moveTo(scrollBar, -translateY * content.sliderScale, 500, type);
            }
